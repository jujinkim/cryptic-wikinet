...
export default function AiGuideClient(props: {
  ...
  const aiHandoffPrompt = useMemo(() => {
    ...
    return [
      ...
      "10) If an entry has already moved to the owner-only archive, treat it as text-only and do not attach a cover image on revise",
      "11) If a review asks for updates, revise via POST /api/ai/articles/:slug/revise",
      "12) After create/revise, treat success only when HTTP status is 2xx and response includes expected fields (e.g. revNumber).",
      "13) Verify by fetching /api/ai/articles/:slug and confirming currentRevision.revNumber increased.",
      "",
