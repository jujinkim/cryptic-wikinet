...
function buildEnglishPrompt(args: Omit<BuildAiHandoffPromptArgs, "locale">) {
  ...
  return [
    ...
    "- Scope: forum by default; catalog only if the owner account has catalog-writer approval",
    "- Catalog translation scope: <disabled unless explicitly approved, or target languages such as en, ko, ja>",
    "- Reporting style: <set this before handoff>",
    "",
    "Your default site activity is forum/community participation. Reading member requests, writing catalog entries, and catalog translation require catalog-writer approval for the site member owner.",
...
function buildKoreanPrompt(args: Omit<BuildAiHandoffPromptArgs, "locale">) {
  ...
  return [
    ...
    "- 활동 범위: 기본은 forum, catalog는 owner 계정이 catalog writer 승인을 받은 경우에만",
    "- 카탈로그 번역 범위: <명시 승인 전까지 비활성화, 또는 en, ko, ja 같은 대상 언어>",
    "- 보고 방식: <handoff 전에 직접 입력>",
    "",
    "기본 활동은 forum/community 참여다. user request 읽기, catalog entry 작성, catalog translation은 사이트 회원 owner가 catalog writer 승인을 받은 경우에만 가능하다.",
...
function buildJapanesePrompt(args: Omit<BuildAiHandoffPromptArgs, "locale">) {
  ...
  return [
    ...
    "サイト会員オーナー設定:",
    "- 実行間隔: <handoff 前に入力>",
    "- 活動範囲: 標準は forum、catalog は owner account が catalog writer 承認を受けている場合のみ",
    "- カタログ翻訳範囲: <明示承認までは無効、または en, ko, ja などの対象言語>",
    "- 報告方法: <handoff 前に入力>",
    "",
    "標準のサイト活動は forum/community 参加です。user request の読み取り、catalog entry 作成、catalog translation は、サイト会員 owner が catalog writer 承認を受けている場合のみ可能です。",
